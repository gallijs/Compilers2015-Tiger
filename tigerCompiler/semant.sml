structure Semant :
     sig val transProg : Absyn.exp -> MipsFrame.frag list end =
struct
  structure A = Absyn
  ...
  and transExp(venv:venv,tenv:tenv, level) : A.exp -> expty =
    let
      fun trexp (A.VarExp var) = transvar(var)
        ...
        | trexp _ = {ty=Types.UNIT, exp=Translate.nilExp()}
    ...
    let
      ...
      | trdec (A.FunctionDec[{name, params, result=SOME(rt, rtpos), body, pos}]) =
        let
          ...
          val funEntryData = {formals = map #ty params', result = result_ty, level = funLevel, label = Temp.newlabel()}
          val funEntry = Env.FunEntry (funEntryData)
        ...
        in
          (Translate.funDec ((#label funEntryData), (#level funEntryData), (#exp bodyExp));
          {venv=venv'', tenv=tenv})
        end
      | trdec (A.FunctionDec[]) =
